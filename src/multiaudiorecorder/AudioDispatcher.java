...
public final class AudioDispatcher implements Runnable {
	...
	public void run() {
		...
		try{
			...
			String message=
                                String.format("Did not skip the expected amount of bytes,  %d skipped, %d expected!",
                                        skipped,bytesToSkip);	
			LOG.warning(message);
			throw new Error(message);
		}
	...
	private int readNextAudioBlock() throws IOException {
		...
		while(!stopped && !endOfStream && totalBytesRead<bytesToRead){
			try{
                            bytesRead = audioInputStream.read(
                                    audioByteBuffer, offsetInBytes + totalBytesRead , bytesToRead - totalBytesRead);
		...
		if(endOfStream){
                    ...
                    if(zeroPadLastBuffer){
                        ...
                        converter.toFloatArray(audioByteBuffer, offsetInBytes, audioFloatBuffer,
                            ...
                            converter.toFloatArray(audioByteBuffer, offsetInBytes,
                                    audioFloatBuffer, offsetInSamples,
                                    totalSamplesRead);
                    }
		}else if(bytesToRead == totalBytesRead) {
			// The expected amount of bytes have been read from the stream.
			if(isFirstBuffer && !zeroPadFirstBuffer){
                            converter.toFloatArray(audioByteBuffer, 0, audioFloatBuffer,
		...
		audioEvent.setOverlap(offsetInSamples);
                audioEvent.setSourceDate(audioByteBuffer);
