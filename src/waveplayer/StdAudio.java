...
public final class StdAudio {
    ...
    public static double[] read(byte[]buffer) {
        byte[] data = buffer;
        int N = data.length;
        double[] d = new double[N/2];
        for (int i = 0; i < N/2; i++) {
...
//        ****************************************************************************************************************
        DataLine.Info targetLine = new DataLine.Info(TargetDataLine.class, ITargetDataLine.DefaultAudioFormat(false));
        Mixer.Info[] mixer = AudioSystem.getMixerInfo();
        ArrayList<Mixer.Info>list = new ArrayList<>();
        ...
        ITargetDataLine data = null;
        try {
            Mixer.Info in = list.get(1);
            System.out.println("name - "+in.getName());
            data = new ITargetDataLine(ITargetDataLine.DefaultAudioFormat(false), in);
            data.start();
        } catch (LineUnavailableException ex) {
            Logger.getLogger(StdAudio.class.getName()).log(Level.SEVERE, null, ex);
        }
        boolean status = true;
        
        while(status){
            byte[] buf = data.get(100);
            StdAudio.play(read(buf));
        }
//        ****************************************************************************************************************
        ...
        for (int i = 0; i < steps.length; i++) {
            double hz = 440.0 * Math.pow(2D, steps[i] / 12.0D);
